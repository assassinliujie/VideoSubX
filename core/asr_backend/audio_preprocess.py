...
def convert_video_to_audio(video_file: str):
    ...
    # 如果 vocal.mp3 已存在，无需生成 raw.mp3（后续分离也会跳过）
    if os.path.exists(_VOCAL_AUDIO_FILE):
        rprint(f"[yellow]⚠️ {_VOCAL_AUDIO_FILE} already exists, skip audio extraction.[/yellow]")
        return
...
def process_transcription(result: Dict) -> pd.DataFrame:
    ...
    for segment in result['segments']:
        ...
        for word in segment['words']:
            # Check word length
            if len(word["word"]) > 30:
                rprint(f"[yellow]⚠️ Warning: Detected word longer than 30 characters, keeping: {word['word']}[/yellow]")
